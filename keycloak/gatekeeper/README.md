...
Создаём клиента
    ...
    Clients -> Create
        ...
        Root URL -> https://application.bart.team
        Save
...
Проверяем.
    ...
    -d "username=user1" \
    -d "password=password" \
    http://keycloak.bart.team/iam/auth/realms/app/protocol/openid-connect/token | jq
...
Посмотрим содержимое токена.
    ...
    http://keycloak.bart.team/iam/auth/realms/app/protocol/openid-connect/token/introspect
...
Например:
    ...
    http://keycloak.bart.team/iam/auth/realms/app/protocol/openid-connect/token/introspect | jq 
